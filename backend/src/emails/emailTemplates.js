...
export function createVerificationEmailTemplate(name, verificationCode) {
  return `
  <!DOCTYPE html>
  <html lang="en">
  <head>
  ...
  <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f5f5f5;">
    ...
    <div style="background-color: #ffffff; padding: 35px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
      ...
      <div style="background: linear-gradient(to right, #36D1DC, #5B86E5); padding: 20px; border-radius: 10px; margin: 30px 0; text-align: center;">
        <p style="color: white; margin: 0; font-size: 14px; margin-bottom: 10px;">Your verification code is:</p>
        <p style="color: white; margin: 0; font-size: 32px; font-weight: bold; letter-spacing: 5px;">${verificationCode}</p>
      </div>
      
      <p style="color: #666; font-size: 14px;">This code will expire in 24 hours. If you didn't request this verification, you can ignore this email.</p>
